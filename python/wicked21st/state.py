    ...
    def are_in_crisis(self, category=None):
        ...
        for k, v in self.items():
            if v["status"]:
                if (
                    category is None
                    or v["category"] == self.graph.category_for_name[category]
                ):
                    result.add(k)
        return result
    ...
    def show(self, size=Graph.GRAPH_PRINT_SIZE):
        ...
        for _id, name in self.graph.node_names.items():
            ...
            digraph.node(
                ...
                shape=shape,
                color=color,
                fillcolor=self.graph.class_for_node[_id],
                style="filled",
            )
...
class TechTreeState(ValidKeysDict):
    ...
    def find_tech(self, type_, suit: str, node: str = None):
        for tech in self.tree.technologies:
            ...
            # else, continue
        raise Exception(
            "Not found: find_tech({} [{}], {}, {})".format(
                Tech.TYPES[type_], type_, suit, node
            )
        )
    ...
    def research_boundary(self):
        result = list()
        researched = set(
            map(lambda t: t.name, self.techs_for_status(TechTreeState.RESEARCHED))
        )
